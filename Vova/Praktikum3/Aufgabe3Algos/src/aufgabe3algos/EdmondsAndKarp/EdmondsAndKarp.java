 ...
 */
package aufgabe3algos.EdmondsAndKarp;
...
public class EdmondsAndKarp {
    ...
    private Queue<Integer> queueOfVertices;
    ...
    public EdmondsAndKarp(AIGraph graph) {
        ...
        this.queueOfVertices = new LinkedList<>();
        init();
    }
     ...
     * @param initialValue
     */
    public EdmondsAndKarp(AIGraph graph, int initialValue) {
        this.graph = graph;
    ...
    /**
     * Gibt einen belibiege Ecke zurück, die noch nicht inspeziert wurde.
     *
     * @return
     */
    private Integer giveMeOneVertex() {
        return queueOfVertices.remove();
    }
        ...
        for (Integer edgeId : workEdges) {
            ...
            if (!(vertexData.isMarked(workVertex))) {
                if (edgeData.getWorkFlow(edgeId) < edgeData.getActualFlow(edgeId)) {
                    ...
                    queueOfVertices.add(workVertex);
    ...
    private List<List<Integer>> edgesAmongQAndNegQ() {
        ...
        List<Integer> markedVerticesList = vertexData.getMarkedList();
        for (Integer vertexId : markedVerticesList) {
            for (Integer edgeId : graph.getIncident(vertexId)) {
                ...
                } else if (!vertexData.isMarked(graph.getTarget(edgeId))) {
    ...
    public String getResultValue() {
        String result = "";
        Map<Integer, String> resMap = graph.getIDtoName();
